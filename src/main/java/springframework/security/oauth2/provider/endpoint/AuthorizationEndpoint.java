...
import org.hzero.oauth.security.event.AuthorizeCodeEvent;
...
public class AuthorizationEndpoint extends AbstractEndpoint implements ApplicationContextAware {
	...
	private String appendAccessToken(AuthorizationRequest authorizationRequest, OAuth2AccessToken accessToken) {
		...
		if (refreshToken != null) {
			vars.put("refresh_token", refreshToken.getValue());
		}
	...
	private String generateCode(AuthorizationRequest authorizationRequest, Authentication authentication)
		...
		try {
            ...
            // Added by xiaowei.zhang 20201113
			ServletRequestAttributes attributes =
				(ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
			Assert.notNull(attributes, "request attributes is null.");
			HttpServletRequest request = attributes.getRequest();

			AuthorizeCodeEvent authorizeCodeEvent = new AuthorizeCodeEvent(request, authorizationRequest, code);
			applicationContext.publishEvent(authorizeCodeEvent);
